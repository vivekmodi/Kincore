...
def classify_ligands(pwd,df):
    ...
    for i in df.index:
        ...
        ligand_label=list()
        backpocket_count=dict()
        frontpocket_count=dict()
        
        
        
        #if 'HUMAN' not in uniprotid:
        ...
        if 'No_ligand' in df.at[i,'Ligand']:
            ...
            continue
        handle1=gzip.open(f'{pwd}/kinasechains_renumber_alignment/{pdbs}.cif.gz','rt')        
        parser1=PDB.MMCIFParser()
        structure1=parser1.get_structure(pdbs,handle1)
        ...
        for items in ligandlist:
            ...
            min_rre4=999;min_hinge=999;dfgcontact=0;dfgoutcontact=0;
                                                ...
                                                if ((residue2.get_id()[0]==' ') or ((residue2.id[0][2:]+'\n') in ignoremodified.readlines())):
                                                    ...
                                                    if (res_id>=106 and res_id<=184) or (res_id>=187 and res_id<=195)  or (res_id>=420 and res_id<=422) or \
                                                        res_id==1011 or res_id==959 or (res_id>=1337 and res_id<=1339):
                                                            #if  (res_id>=1337 and res_id<=1338) and dfgcontact==1:   #Do not count dfg contact twice
                                                            #    continue
                                                            #if res_id==1339 and phecontact==1:
                                                            #    continue
            ...
            if min_rre4!=999 or min_hinge!=999:
                ...
                elif min_hinge>=5 and backpocket_count[ligandname+':'+ligandid]>=3:
