...
def chain_color(df):
    for i in df.index:    
        if df.at[i,'Dihedral']=='BLAminus':
            df.at[i,'Color']=str('(179,215,229)')         #Cyan - ##b3d7e5 nice color
        ...
        if df.at[i,'Dihedral']=='BLBplus':              
            df.at[i,'Color']=str('(199,233,175)')          #LightGreen - nice color - #c7e9af
        ...
        if df.at[i,'Spatial']=='DFGin' and df.at[i,'Dihedral']=='None':
            df.at[i,'Color']=str('(230,230,230)')         #LightGrey - #e6e6e6 nice color
