...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  9 13:19:15 2020
...
def identify_state(pwd,pdbfilename,align,user_chain,user_lys,user_glu,user_phe):
    ...
    if '.gz' in pdbfilename.lower():
        handle=gzip.open(f'{pwd}/server/uploads/{pdbfilename}','rt')
    else:
        handle=open(f'{pwd}/server/uploads/{pdbfilename}','r')
            ...
            for chain in model:
                ...
                if conf_df.at[index,'Group']=='None':
                    ...
                    if conf_df.at[index,'Lys_restype']=='X':
                        ...
                        conf_df=compute_distance(pwd,pdbfilename,index,conf_df,structure)
                        conf_df=compute_dihedrals(pwd,pdbfilename,index,conf_df,structure)
