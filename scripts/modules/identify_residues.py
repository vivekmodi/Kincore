...
def identify_residues(pwd,pdbfilename,index,conf_df):
    ...
    hmm_result=SearchIO.read(f'{pwd}/server/{pdbfilename[0:-4]}_{model_id}_{chain_id}_{group}.hmmer.txt',format='hmmer3-text')
    for hits in hmm_result:     #extract hit from alignment in HMM output file
        for hsps in hits:
            col_num=0;hmm_index=hsps.query_start;hit_index=int(hsps.hit_start+first_res-1)
